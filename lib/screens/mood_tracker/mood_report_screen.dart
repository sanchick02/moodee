...
class _MoodReportScreenState extends State<MoodReportScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    List<List<MoodTracker>> generateMonthlyMoodData(List<MoodTracker> moodData) {
    List<List<MoodTracker>> monthlyMoodData = List.generate(12, (_) => []);
      for (var mood in moodData) {
        int month = DateTime.parse(mood.date).month;
        monthlyMoodData[month - 1].add(mood); // Subtracting 1 because months are 1-indexed in DateTime
      }
    ...
    String mostFrequentMood(List<MoodTracker> monthMoodData) {
      ...
      return sortedMoodCounts.isNotEmpty ? sortedMoodCounts.first.key : 'Unknown';
    }
                          ...
                          children: [
                                ...
                                for (var i = startValue; i < endValue; i++)
                                  Positioned(
                                    ...
                                    child: GestureDetector(
                                      ...
                                      child: CircleAvatar(
                                        radius: monthlyMoodData[i].length.toDouble(),
                            ...
                            SizedBox(
                              ...
                              child: ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  return Padding(
                                    padding: const EdgeInsets.only(right: 15.0, left: 17),
                                    child: Text(
                                      quarterOne ? months[index] : julyOnwards[index]
                                    ),
                                  );
                                }),
                            )
                  ...
                  children: [
                    ...
                    IconButton(
                      ...
                      Column(
                        children: [
                          Text(
                            quarterOne ? 'First Half of 2024' : 'Second Half of 2024',
                            style: AppFonts.normalRegularText,
                          ),
                          const SizedBox(
                            width: 250,
  ...
  double _calculatePositionY(String moodType) {
    // Implement your logic to map mood types to Y-axis positions
    switch (moodType) {
      case 'Happy':
        return 0.0; // Top of the grid
      case 'Loved':
        return 50.0; // Middle of the grid
      case 'Excited':
        return 100.0;
      case 'Emotionless':
        return 150;
  ...
  double _calculatePositionX(int monthIndex) {
    // Adjust the spacing based on your preference
    return monthIndex * 48.0;
  }
